...
module.exports.definition = {
  set(v) {
    ...
    if (parsers.hasVarFunc(v)) {
      ...
      for (const [key, parser] of module.exports.positionShorthandFor) {
        this._setProperty(key, "");
      ...
      if (obj === "") {
        ...
        const borderValue = [...Object.values(obj)].join(" ");
        for (const [key] of module.exports.positionShorthandFor) {
          this._setProperty(key, borderValue);
        }
        this._setProperty("border-image", "none");
        this._setProperty("border", borderValue);
      }
    }
  },
  get() {
    const val = this.getPropertyValue("border");
    if (parsers.isGlobalKeyword(val) || parsers.hasVarFunc(val)) {
      return val;
    }
    const subVal = this._shorthandGetter("border", module.exports.shorthandFor, initialValues);
