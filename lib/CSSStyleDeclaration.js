  ...
  get cssText() {
    ...
    for (let i = 0; i < this._length; i++) {
      ...
      if (priority === "important") {
        ...
        if (implicitPositionProperties.has(property)) {
          const implicitProperties = implicitPositionProperties.get(property);
          const relatedProperties = [...implicitProperties.keys()];
          if (!relatedProperties.every((key) => properties.has(key))) {
            properties.set(property, [property, value, null]);
          }
        } else {
          properties.set(property, [property, value, null]);
        }
...
Object.defineProperties(CSSStyleDeclaration.prototype, {
  ...
  _implicitPositionSetter: {
     ...
     * @param {Array.<string>} positions
     */
    value(property, val, position, positions = []) {
      const implicitProperties = implicitPositionProperties.get(property);
      const index = positions.findIndex((i) => i === position);
      const firstKey = implicitProperties.keys().next().value;
      const isInitialValue = val === implicitProperties.get(firstKey);
      for (const [key, initialValue] of implicitProperties) {
        const parsedValues = [];
        const propertyValue = this.getPropertyValue(key);
        if (propertyValue && propertyValue !== initialValue) {
          const propertyValues = splitValue(propertyValue);
          const replaceValue = isInitialValue ? initialValue : val;
          switch (propertyValues.length) {
            case 1: {
              const [val1] = propertyValues;
              switch (index) {
                case 0: {
                  parsedValues.push(replaceValue, val1, val1);
                  break;
                }
                case 1: {
                  parsedValues.push(val1, replaceValue, val1, val1);
                  break;
                }
                case 2: {
                  parsedValues.push(val1, val1, replaceValue);
                  break;
                }
                case 3: {
                  parsedValues.push(val1, val1, val1, replaceValue);
                  break;
                }
                default: {
                  return;
            ...
            case 2: {
              const [val1, val2] = propertyValues;
              switch (index) {
                case 0: {
                  parsedValues.push(replaceValue, val2, val1);
                  break;
                }
                case 1: {
                  parsedValues.push(val1, replaceValue, val1, val2);
                  break;
                }
                case 2: {
                  parsedValues.push(val1, val2, replaceValue);
                  break;
                }
                case 3: {
                  parsedValues.push(val1, val2, val1, replaceValue);
                  break;
                }
                default: {
                  return;
            ...
            case 3: {
              const [val1, val2, val3] = propertyValues;
              switch (index) {
                case 0: {
                  parsedValues.push(replaceValue, val2, val3);
                  break;
                }
                case 1: {
                  parsedValues.push(val1, replaceValue, val3, val2);
                  break;
                }
                case 2: {
                  parsedValues.push(val1, val2, replaceValue);
                ...
                case 3: {
                  parsedValues.push(val1, val2, val3, replaceValue);
                  break;
                }
                default: {
                  return;
        ...
        if (parsedValues.length) {
          const parsedValue = parsedValues.join(" ");
          if (isValidPropertyValue(key, parsedValue)) {
            this._setProperty(key, parsedValue);
          }
        }
      }
    },
